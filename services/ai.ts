...
export const searchPlace = async (
  ...
  let locationStr = "user's current location";
  ...
  const prompt = `
    Find restaurants matching the name "${query}" near ${locationStr}.
  ...
  // Note: responseSchema/responseMimeType is NOT supported when using googleMaps tool.
...
export const findBestPlace = async (
  ...
  const prompt = `
    ...
    2. If no favorites match, use Google Maps to find 3 highly rated "${cuisine}" places near ${locationStr} suitable for "${mode}".
...
export const getRouletteOptions = async (
    ...
    const prompt = `
      Find 6 diverse, highly-rated restaurants near ${locationStr}.
